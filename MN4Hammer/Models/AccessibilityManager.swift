...
class AccessibilityManager {
    ...
    /// 根据提供的AXUIElement 得到 孩子对象表或nil
    func getElementChildren(in element: AXUIElement) -> [AXUIElement]? {
        var value: CFTypeRef?
    ...
    /// 根据提供的对象得到parent或nil
    func getElementParent(in element: AXUIElement) -> AXUIElement? {
        var value: CFTypeRef?
        let error = AXUIElementCopyAttributeValue(element, kAXChildrenAttribute as CFString, &value)
        guard error == .success, value != nil else {
    ...
    func getFocusedWindow(from application: AXUIElement) -> AXUIElement? {
        ...
        guard getElementRole(window) == "AXWindow" else {
            return nil
        }
    ...
    func getElementSize(for element: AXUIElement) -> CGSize? {
    ...
    // 获取元素Frame, CGRect格式
    func getElementFrame(for element: AXUIElement) -> CGRect? {
